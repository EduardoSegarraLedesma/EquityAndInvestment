...
public class WebScrapping {
    ...
    public String searchForCompanyStockPricewithSymbol(String symbol) {
        List<Map<String, String>> companyList = this.getCompanyData();

        for (Map<String, String> company : companyList) {
            if (company.get("companyCode").equals(symbol)) {
                return company.get("stockPrice");
            }
        }
    ...
    public String searchCompanyCode(String name) {
        List<Map<String, String>> companyList = this.getCompanyData();

        for (Map<String, String> company : companyList) {
            if (company.get("companyName").equals(name)) {
                return company.get("companyCode");
            }
        }
    ...
    public float searchForCompanyStockPriceFloat(String name) {
        String aux = this.searchForCompanyStockPrice(name);
        float solution = this.parseStringToFloat(aux);
        return solution;
    }

    public float searchForCompanyStockPriceFloatWithSymbol(String symbol){
        List<Map<String, String>> companyList = this.getCompanyData();
        String aux = null;
        for (Map<String, String> company : companyList) {
            if (company.get("companyCode").equals(symbol)) {
                aux = company.get("stockPrice");
            }
        }
        if (aux != null){
            return this.parseStringToFloat(aux);
        }
        return 0;
    }
    ...
    private float parseStringToFloat(String name) {
        if (name == null || name.isEmpty()) {
            throw new IllegalArgumentException("Input string cannot be null or empty.");
        }

        // Remover el signo de dólar y comas que puedan existir en la cadena
        String numericString = name.replace("$", "").replace(",", "");

        try {
            // Convertir la cadena limpiada a un float
            return Float.parseFloat(numericString);
        } catch (NumberFormatException e) {
            // Manejar el caso en que la cadena no contenga un formato numérico válido
            throw new IllegalArgumentException("Input string must be a valid dollar amount.");
        }
    }
